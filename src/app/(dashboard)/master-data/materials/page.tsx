...
export default function MaterialsPage() {
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      const res = await batchDeleteMaterialsAction(idsToDelete);
      if (res.success) {
        toast.success(`${idsToDelete.length} Material berhasil dihapus`);
        setSelectedIds(new Set());
        setIsBulkDeleteOpen(false);
        await fetchMaterials();
      } else {
        toast.error(res.error || 'Gagal menghapus material terpilih');
      }
    } catch (error) {
      console.error('Failed to bulk delete materials', error);
      toast.error('Gagal menghapus material terpilih');
    } finally {
        ...
        <div className="flex items-center justify-between px-4 py-2.5 bg-red-50/80 dark:bg-red-950/40 border border-red-200 dark:border-red-900/50 rounded-lg text-[13px] animate-in fade-in">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-red-900 dark:text-red-300">
              {selectedIds.size} Material dipilih
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Button
            ...
            >
              ...
              Hapus {selectedIds.size} Material Terpilih
            </Button>
          </div>
        </div>
      )}
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-[13px] pt-2">
              Apakah Anda yakin ingin menghapus <span className="font-semibold text-foreground">{selectedIds.size} Material terpilih</span>?
